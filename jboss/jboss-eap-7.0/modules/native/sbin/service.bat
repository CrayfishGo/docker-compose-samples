...
rem defaults
set SHORTNAME=JBossServer
set DISPLAYNAME=JBossServer
rem NO quotes around the description here !
set DESCRIPTION=JBoss Enterprise Application Platform 7
...
:cmdUsage
echo Red Hat JBoss EAP 7 Service Script for Windows
echo Usage:
echo(
...
if /I "%IS_DOMAIN%" == "true" (
  ...
  if "%CONFIG%"=="" set CONFIG=standalone-full.xml
  if "%START_SCRIPT%"=="" set START_SCRIPT=standalone.bat
  set STARTPARAM="/c#set#NOPAUSE=Y#&&#!START_SCRIPT!#-Djboss.server.base.dir=!BASE!#--server-config=!CONFIG!#-b=0.0.0.0"
